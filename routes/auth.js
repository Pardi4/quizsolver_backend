...
const { authLimiter, qrLimiter, generalLimiter } = require('../middleware/rateLimiter');
...
function parseExtensionRedirect(value) {
  if (!value || typeof value !== 'string') return '';
  try {
    const url = new URL(value);
    const isChromeIdentityRedirect = url.protocol === 'https:'
      && /^[a-p]{32}\.chromiumapp\.org$/i.test(url.hostname);
    if (!isChromeIdentityRedirect) return '';
    return {
      id: url.hostname.replace(/\.chromiumapp\.org$/i, '').toLowerCase(),
      url: value.substring(0, 500)
    };
  } catch {
    return '';
  }
...
function safeExtensionRedirect(value) {
  const parsed = parseExtensionRedirect(value);
  if (!parsed) return '';
  const allowedIds = allowedChromeExtensionIds();
  if (allowedIds.length > 0 && !allowedIds.includes(parsed.id)) return '';
  if (process.env.NODE_ENV === 'production' && allowedIds.length === 0) return '';
  return parsed.url;
}

function extensionTokenRedirect(baseUrl, token) {
...
router.post('/qr/generate', qrLimiter, async (req, res) => {
  try {
    ...
    console.error('qr/generate error:', error.message || error);
    res.status(500).json({ error: 'Could not create QR login session.' });
  }
});


router.get('/qr/status/:sessionId', generalLimiter, async (req, res) => {
  try {
    ...
    console.error('qr/status error:', error.message || error);
    res.status(500).json({ error: 'Could not check QR session status.' });
  }
});
...
router.post('/qr/confirm', authMiddleware, authLimiter, async (req, res) => {
  try {
    ...
    if (!session) {
      return res.status(404).json({ error: 'QR session expired or not found.' });
    }
    ...
    if (session.status === 'confirmed') {
      return res.status(409).json({ error: 'QR session already confirmed.' });
    }

    
    if (!codeMatches(session.secretHash, secret)) {
      return res.status(403).json({ error: 'Invalid QR secret.' });
    }

    
    const token = generateToken(req.user._id, true);
    ...
    res.json({
      success: true,
      message: 'Login confirmed. The extension will be logged in shortly.',
      displayName: req.user.displayName || req.user.email
    });
  } catch (error) {
    console.error('qr/confirm error:', error.message || error);
    res.status(500).json({ error: 'Could not confirm QR login.' });
  }
});
